...
public class MeshSplitter : MonoBehaviour
{
    ...
    public void SplitMesh(Vector3 planePoint1, Vector3 planePoint2, Vector3 planePoint3, Transform playerTransform)
    {
        ...
        Plane cuttingPlane = CalculateCuttingPlane(planePoint1, planePoint2, planePoint3);
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            ...
            subParts.Add(GenerateMesh(parts[i], cuttingPlane, true));
            subParts.Add(GenerateMesh(parts[i], cuttingPlane, false));
        }
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            if (parts[i].Vertices.Length != 0)
            {
                parts[i].MakeGameobject(this);
                parts[i].GameObject.GetComponent<Rigidbody>().AddForceAtPosition(playerTransform.forward * ExplodeForce, transform.position);
            }
        }
    ...
    private Plane CalculateCuttingPlane(Vector3 planePoint1, Vector3 planePoint2, Vector3 planePoint3)
    {
        ...
        origin.transform.eulerAngles = gameObject.transform.eulerAngles;
        origin.transform.localScale = gameObject.transform.localScale;

        p1.transform.position = planePoint1;
        p2.transform.position = planePoint2;
        p3.transform.position = planePoint3;

        p1.transform.parent = origin.transform;
        p2.transform.parent = origin.transform;
        p3.transform.parent = origin.transform;

        origin.transform.eulerAngles = Vector3.zero;
        origin.transform.localScale = new Vector3(1, 1, 1);

        planePoint1 = p1.transform.position;
        planePoint2 = p2.transform.position;
        planePoint3 = p3.transform.position;

        Destroy(origin);

        Vector3 center = (planePoint1 + planePoint2 + planePoint3) / 3f;

        Plane plane = new Plane(planePoint1, planePoint2, planePoint3);
        return new Plane(plane.normal, center - gameObject.transform.position);
    }
    ...
    public class PartMesh
    {
        ...
        public void MakeGameobject(MeshSplitter original)
        {
            ...
            var meshSplitter = GameObject.AddComponent<MeshSplitter>();
            meshSplitter.ExplodeForce = original.ExplodeForce;

            var meshDestroy = GameObject.AddComponent<MeshDestroy>();
            meshDestroy.CutCascades = 5;
            meshDestroy.ExplodeForce = 1000;
